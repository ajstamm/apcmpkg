...
<!-- Outline
Intro
- For my study, I joined environmental and birth defects surveillance data
- Today, will focus on environmental datasets and why I chose mine
- Strengths & limitations of each & determining measures
- Issues I encountered joining environmental & birth defects data
- Why issues mattered (how they affect results) and how I handled them
- Other considerations (eg. NYC)
- Briefly, my findings

Environmental data
- Considerations
    - Geography (tract, county, grid, buffer)
    - Frequency (how often, how continuous)
    - Location & altitude (o3 monitor in Demetral Field, east of Lake Mendota near railway, different exposure from Waismann Center to south but on WI tracker represents all fo Dane County - https://dhsgis.wi.gov/DHS/EPHTracker/#/all/Air%20Quality/airQualityIndex/NOTRACT/204/83)
- Air pollutants
    - Monitoring (sparse, may be on towers/buildings)
    - Modeled (may be infrequent, can't measure exact exposure)
- Green space
...
# w <- tigris::area_water(state = "NY", county = "Chautauqua")
# plot(w$geometry, border = "blue", col = "lightblue")
# w |> dplyr::arrange(desc(AWATER)) |> head()
...
* *Resolution:* Points spread far apart
* *Geography:* Mostly in urban areas
...
<!-- \begin{figure}[H] -->
<!-- \includegraphics[width=1\textwidth, height=!]{images/airmonitors.jpg} -->
<!-- \end{figure} -->
\centering
\footnotesize
...
* Cloud cover affects usability
* Hard to differentiate land types
* Vary by season
...
#          xaxt = 'n', # legend.args=list(side = 1, line = 0), 
...
legend(x='bottomleft', legend = ntemp$Class, fill = ntemp$Color, cex = 1.5, bg = "white")
...
legend(x='bottomleft', legend = nt2$Class, fill = nt2$Color, cex = .5, bg = "white")
